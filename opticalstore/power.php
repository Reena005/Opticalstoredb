...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_deduction'])) {
    ...
    $eye_left = floatval($_POST['eye_left']);
    $eye_right = floatval($_POST['eye_right']);
    $deduction_amount = floatval($_POST['deduction_amount']);
    $deduction_date = $_POST['deduction_date'];
    $notes = trim($_POST['notes']);

    if ($customer_id && ($eye_left || $eye_right)) {
        $query = "INSERT INTO CustomerEyePowerProblem 
                  (customer_id, EyePowerLeft, EyePowerRight, DeductionAmount, DeductionDate, Notes)
                  VALUES ($1, $2, $3, $4, $5, $6)";
        ...
        $error = "❌ Customer and at least one eye power are required.";
    }
}
?>
...
<div class="container">
    ...
    <form method="POST">
        ...
        <div class="form-control">
            ...
            <input type="number" step="0.01" name="deduction_amount" placeholder="Amount">
        </div>
